...
const validateForm = () => {
  ...
  if (formData.value.title.length > 100) {
    errors.value.title = '標題長度不可超過 100 字'
    return false
  }
  return true
}
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const response = await fetch('/index.php/api/admin/news', {
      method: 'POST',
      headers: {
    ...
    <div class="flex-1 flex flex-col">
      <main class="flex-1 pt-24 px-4 md:px-6">
        <div class="max-w-7xl mx-auto bg-white p-4 md:p-6">
          ...
          <h2 class="text-lg md:text-2xl font-semibold text-gray-800 mb-4 md:mb-6">新增消息</h2>
